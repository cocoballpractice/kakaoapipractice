...
class DirectionServiceTest extends Specification {
    ...
    private List<PharmacyDto> pharmacyList // 테스트에 필요한 리스트


    def setup() { // 테스트 실행 시 리스트에 값 채워넣기
    ...
    def "buildDirectionList - 결과 값이 거리 순으로 정렬이 되는지?"() {
        given:
    ...
    def "buildDirectionList - 결과 값이 정해진 반경 10 km 내에 검색이 되는지?"() {
        given:
        pharmacyList.add(
                PharmacyDto.builder()
                        .id(3L)
